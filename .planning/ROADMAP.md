...
| 38.1. Member Workspaces — Active-Workspace UX, Contracts & Authority, Audit | 0/0 | **UNBLOCKED 2026-09-09** — the entire remediation chain it waited on is complete: 38.0.1 (16/16, shipped + verified), 38.0.2 (17/17, verified) and 38.0.3 (6/6, migrations 208/209/210 applied and verified, `no_block` proven unreachable over HTTP). **Not yet planned (0/0)** and the row carries no scope detail beyond its title, so start with `/gsd-discuss-phase 38.1` rather than planning blind. Independent of the RLS smoke sessions — only the D-56 flip is gated on those. | - |
| 38.2. Member Workspaces — Org Billing, Beta Rollout & Doctrine Docs | 0/0 | **UNBLOCKED 2026-09-09** on the same grounds as 38.1 — 38.0.1/38.0.2/38.0.3 are all complete. **Not yet planned (0/0).** Migrations **211-212** are reserved for it in the LIVE MIGRATION LEDGER. **Caveat specific to this phase:** its *Beta Rollout* portion depends on the D-56 workspace-access switch, which is still OFF and is gated on the RLS smoke sessions (`docs/verification/BETA-RLS-SMOKE-SESSION.md`) plus the plan 38.0.2-17 checkpoint. The Org Billing and Doctrine Docs portions are not gated on any of that. | - |
...
| **201–202** | Playbook rich-content model R1 (Codex) | **PROMOTED 2026-09-09, unapplied and HUMAN-GATED.** `supabase/migrations/201_playbook_rich_documents.sql`, `supabase/migrations/202_playbook_reading_operations.sql`. Apply only through the owner-run Playbook preflight and sequence; both reminder RPCs fail closed until 207 exists. |
...
| **204** | Playbook review threads (Codex) | **PROMOTED 2026-09-09, unapplied and HUMAN-GATED.** `supabase/migrations/204_playbook_review_threads.sql`. Depends on 201–202. Claimed while free; the later Phase 38.2 reservation onto 204 was an error in this ledger, now corrected. |
| **205** | Playbook change broadcasts (Codex) | **PROMOTED 2026-09-09, unapplied and HUMAN-GATED.** `supabase/migrations/205_playbook_change_broadcasts.sql`. Depends on 201, 202, and 204. |
| **206** | Playbook enablement platform, R17–26 (Codex) | **PROMOTED 2026-09-09, unapplied and HUMAN-GATED.** `supabase/migrations/206_playbook_enablement_platform.sql`. Depends on 201, 202, 204, and 205. |
| **207** | Playbook operational v1, R27–31 (Codex) | **PROMOTED 2026-09-09, unapplied and HUMAN-GATED.** `supabase/migrations/207_playbook_operational_v1.sql`. Depends on 201, 202, 204, 205, and 206. All three functions use `SET search_path = ''`; browser execution is revoked explicitly and callable RPCs are granted only to `service_role`. Features default disabled and emergency-disabled. |

...
| **210** | Phase 38.0.3 Tier 3 — `no_block` relocation (plan 05) | **RESERVED, not yet authored.** 208/209 are applied (D4 satisfied). **Blocked on a DEPLOY, not on a migration:** plan 04 removed the app's only `no_block` `.rpc()` call and was pushed to `main` 2026-09-09 (`26f5e2bc`) — 210 may not be applied until that build is live, because PostgREST routes only to exposed schemas and relocating first 404s Green Room placement activation on the first live request. Plans 05 and 06 were renumbered from 209→210 on 2026-09-09; their frontmatter had gone stale when Codex took 207. |

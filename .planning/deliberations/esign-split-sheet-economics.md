...
- **Financial:** a wrong field forces void-and-re-mint. If the mistake was the email itself, the corrected address reads as a NEW recipient under AM-2c's cap logic, silently consuming an allowance slot on what is actually a data-entry fix, not new outreach.
- **Legal:** the pre-signature review prompt (`lib/split-sheets/agreement.ts`, `PRE_SIGNATURE_REVIEW_PROMPT`) already tells signers these details "flow into your PRO and publisher registrations." A wrong legal name or PRO isn't cosmetic — it's bad data landing in a real rights registration, on operative language counsel has now reviewed and approved (P17-09a, 2026-07-21). The only current recovery path — decline, then email the sender out-of-band — is slow, easy to skip, and leaves no record tying the correction to the document.

**Not yet scoped or planned.** Phase 18's 18-01 already lets the INITIATOR edit party data pre-execution with a consensus-reset — the nearest existing precedent — but nothing lets a named PARTY edit their own row. The natural extension is recipient-side self-service correction of one's own fields (not the split percentage, not another party's data) during the approve/counter step, still bounded by `lib/split-sheets/lifecycle.ts`'s freeze boundary once a sheet reaches `esign_pending`/`executed`. Whether a self-correction should reset that one party's approval (mirroring the initiator-edit consensus-reset) or be allowed freely pre-approval is an open design question for whoever plans this.

**Action:** fold into Phase 18 planning (natural fit alongside 18-01/18-02's living-draft and recipient-locker work) or scope as its own quick task — not decided yet. Recorded here so it isn't lost; the live checkpoint continues unblocked in the meantime, since today's test data is fictional and low-stakes by design.

...
- **B2 — unsubscribe does not unsubscribe.** `app/unsubscribe/page.tsx` renders "You've unsubscribed" on token load; the only mutation is resubscribe → `unsubscribed_at` is never set → false opt-out + the broadcast's opt-out filter has nothing to exclude. **Fix (Fix-2):** token-authenticated `POST /api/waitlist/unsubscribe` that sets `unsubscribed_at`; page calls it on load, shows confirmation only on success.

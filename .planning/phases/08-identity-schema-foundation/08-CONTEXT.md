...
Pure schema/migration foundation phase — no UI surface (except one small internal admin route, see D-05). Extends `artist_profiles` into the single unified member-identity table (adding `member_type`, `pronouns`, `banner_url`, `open_to`, `featured_project_id`, `search_vector` — note `location` and `bio` already exist), stands up `connections` and `blocks` tables with RLS, extends `notifications` with actor-snapshot columns + realtime publication, adds `dm_thread_reads`, creates a `no_block()` SECURITY DEFINER helper, retrofits column-level privilege lockdown onto `artist_profiles`, and adds a `reserved_handles` table. Success criteria are locked in `.planning/ROADMAP.md` §"Phase 8" — this discussion clarifies HOW, not whether. No user-facing requirement is mapped to this phase (per REQUIREMENTS.md traceability note); every Phase 9–13 requirement depends on this schema.
...
- **D-08:** The existing `industry_roles` TEXT[] column (already on `artist_profiles`, used today for split-sheet auto-fill) remains the **profile-level display-badge list** — freely editable, independent of `member_type`. A `member_type='artist'` account can self-tag `["Recording Artist", "Producer"]`; a `member_type='industry'` account can self-tag `["A&R", "Songwriter"]`. Same identity row, same array column, no new table needed.
...
- **D-11:** Column classification —
  - **PUBLIC** (readable by anyone): `artist_name`, `genres`, `location`, `bio`, `career_stage`, `instagram_handle`, `threads_handle`, `tiktok_handle`, `spotify_url`, `monthly_listeners`, `total_streams`, `industry_roles`, `handle`, and all new Wave 4 columns (`member_type`, `pronouns`, `banner_url`, `open_to`, `featured_project_id`, `search_vector`).

...
- **D-04 [informational]:** Curators (the fully separate Wave 3 magic-link account type, isolated from `artist_profiles`) **stay deliberately separate** — explicitly out of scope for this phase. Do not fold curator into this capability-grant model. No plan task implements this by design — it is a scope exclusion, not a work item. See `<deferred>` below.
...
- **D-13 [informational]:** **Capability revocation is explicitly out of scope for this phase.** Revoking a grant (e.g. a fraudulent industry claim) is Trust & Safety territory — Phase 13's block/report domain, not an identity-model concern. Admins can revoke manually via direct DB action until real volume justifies building revocation UI. No plan task implements this by design — it is a scope exclusion, not a work item. See `<deferred>` below.

### Server-Side Enforcement (added after research, 2026-07-07)
- **D-14:** Research found a real pre-existing gap: today's artist/industry separation is nav-only — `/api/antenna/opportunities` (POST) and other industry-only routes have **zero server-side capability check**; any authenticated account can already hit them directly regardless of what the sidebar shows. Since this phase makes "capability" a real, requestable, combinable concept (not just a fixed signup-time value), it must **add real server-side capability checks** to any route it newly makes dual-accessible — not just nav-hiding (D-08 governs UI only; D-14 governs the underlying routes). At minimum: opportunity-posting routes gain a `hasCapability(profileId, 'industry')` check. Confirmed via explicit user decision, not left as nav-only status quo.

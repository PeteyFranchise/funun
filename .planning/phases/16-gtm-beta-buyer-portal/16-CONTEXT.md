...
- **D-18b (added 2026-07-18):** Two hard requirements for ANY e-sign surface Funūn ships, now and later: (1) **seamless embedding** — signing never leaves the Funūn experience, no vendor-hosted redirects; (2) **mobile-first** — the signing surface must be genuinely easy on a phone (studio scenario: collaborators with only their phones). Convenience on mobile is the market wedge. Architecture: **dual-provider behind lib/esign/provider.ts** — SignWell serves admin-initiated sync licenses (16-09; hosted-provider legal defensibility is worth the cost on revenue transactions); a cost-optimized embedded provider (DocuSeal front-runner, unverified) will serve artist split-sheet volume later, per `.planning/deliberations/esign-split-sheet-economics.md`.
- **D-18c (2026-07-20 — SUPERSEDES the dual-provider split in D-18a/D-18b):** **One provider: DocuSeal, for every Funūn document type including sync licenses.** D-18b split providers on the premise that SignWell offered better legal defensibility for revenue transactions — **that premise was never verified.** SignWell was chosen before DocuSeal was evaluated, and no SignWell certificate was ever inspected. DocuSeal is verified in depth (live 3-signer async test; certificate carrying dual SHA256, per-signer IP/session/user-agent/timezone, email-verified flag, full timestamped event log, ESIGN/UETA/eIDAS positioning; API-completed signatures distinguished from hand-signed). It is also cheaper at every volume ($0.20 vs $0.85/completion), already paid for and white-labeled, and its mobile embed tested better (spike 006b: SignWell had first-paint horizontal overflow and small on-document tap targets). Consequence: 16-09 writes no adapter and needs no new credentials — it reuses 17-06's. **`lib/esign/provider.ts` stays**, so this is not lock-in; adding a provider back is one adapter. Accepted risk: vendor concentration. Open verification: sequential signing (`order: 'preserved'`) is unexercised and is 16-09's first checkpoint.

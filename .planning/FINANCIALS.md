...
|---|---|---|---|
| **DocuSeal** — ALL e-signatures (split sheets, sync licenses, future contract-library types) | **$20/user/mo + $0.20 per completed document** | ✅ verified; account upgraded to Pro 2026-07-20 | Bills per *completed* doc. **Voided/archived-before-completion envelopes do NOT bill** (verified live during the provider gate — hence `VOIDED_ENVELOPES_COUNT_TOWARD_CAP = false`). A multi-party sheet is ONE completion regardless of signer count — a 5-writer sheet costs $0.20, not $1.00. |
| ~~SignWell~~ — sync-license e-signatures | ~~25 free/mo, then $0.85/doc~~ | ❌ **DROPPED (D-18c, 2026-07-20)** | Never activated. Sync licenses now use the same DocuSeal account — one vendor for every document type. The dual-provider split rested on an unverified assumption about SignWell's legal defensibility; DocuSeal's certificate was inspected in depth and is cheaper at every volume. **Saves a second vendor relationship, a second credential set, and a second webhook.** |

...
| R3 | Phase 31.1 | 31.1-01 (migration live), 31.1-02 (engine done), 31.1-04 (room + live render done); 31.1-05 pending | In progress — `lib/client-partners/health.ts` computeHealth() (31.1-02) now wired to real data via `lib/client-partners/signals.ts` + real `HealthChip` render + the D-31.1-09 executed-license stamping route (31.1-04-SUMMARY.md); Health Rules config screen (thresholds UI) not yet built |
| R6 | Phase 31.1 | 31.1-01 (migration live), 31.1-03 (aggregation done), 31.1-04 (tower render + hide-not-filter done), 31.1-06 (assign panel + self-assign) | Impl complete — the richer assign panel (D-31.1-05: AE search + load + health mix + required note) is mounted in the leadership tower, PATCHes the widened ae route (leadership self-assign admitted), `tsc`/full suite green (31.1-06-SUMMARY.md); owner UAT pending |
| R4 | Phase 31.1 | 31.1-01 (migration live); 31.1-05 pending | In progress — migration 128's `health_rules_config` seeded singleton (D-31.1-03/08 thresholds/keeps-warm/prospect image) applied to prod (31.1-01-SUMMARY.md); the Health Rules settings screen + config/stage routes not yet built |
| R7 | Phase 31.1 | 31.1-01 (migration live); 31.1-04, 31.1-06 (assign route + onboarding task) | Impl complete — assigning now requires a handoff note (400 if missing) and auto-inserts an `onboarding_tasks` row (seeded checklist + note) in the AE's queue, surfaced read-only on their My tab via `OnboardingTasksPanel` (31.1-06-SUMMARY.md); owner UAT pending |
| R8 | Phase 31.1 | 31.1-01 (migration live); 31.1-06 (structural handoff) | Impl complete — assignment fires the full D-07 handoff (relationship-log `kind:'assignment'` entry, onboarding task, notification + best-effort Resend intro email) after the `ae_user_id` write commits; every side effect independently `.catch()`-wrapped so a Resend/DB failure never blocks the response (31.1-06-SUMMARY.md); owner UAT pending (Resend prod sender unverified per project memory — delivery failure is by-design non-blocking) |
| R14 | Phase 31.1 | 31.1-01 (migration live); 31.1-07 (route + UI) | Impl complete — GET/PUT/POST `/api/admin/client-partners/[orgId]/game-plan` (load-or-seed, save, log-conversation) + `GamePlanPanel` mounted in the person workspace; "Log conversation" writes "X of N covered" + notes to the relationship log (incl. the 0-covered edge) and retires the plan (31.1-07-SUMMARY.md); owner UAT pending |
| R13 | Phase 31.2 | 31.2-01 (migration 132 live), 31.2-02 (pure audible-time accumulator + `engagement.ts` aggregation), 31.2-05 (public write route + `SelectsPlayer` telemetry wiring), 31.2-10 (AE per-Selects readout + leadership rollup reads) | Impl complete — a genuine play/pause/seek-aware audible-time accumulator (never a naive wall-clock timer) writes raw per-track delta rows via a rate-limited, clamp-defended public route; the AE's own-Selects-scoped `EngagementPanel` and the leadership-only `engagement-rollup` route both SUM those raw rows at read time (never a stored total) into per-track plays/audible-seconds/qualified-listens(≥30s)/replays + a Selects/per-AE summary; staff-only throughout — a contract test locks the public player render path out of the engagement tables (31.2-05-SUMMARY.md, 31.2-10-SUMMARY.md); owner UAT pending (play a Selects preview with pause/seek, confirm audible seconds match real listening, confirm the leadership rollup reflects it) |

**Coverage (Phase 31 — Slice 1):**

- Phase 31 Slice-1 requirement IDs: **6** (R1, R2, R5, R10, R11, R12). Slice-2 IDs (R3, R4, R6–R9, R13, R14) → Phase 31.1: R6/R7/R8/R14 impl complete (31.1-06-SUMMARY.md, 31.1-07-SUMMARY.md, 2026-08-24); R3/R4 in progress per rows above; R13 impl complete (31.2-10-SUMMARY.md, 2026-08-24) — Phase 31.2; R9 deferred to Phase 31.2 (CONTEXT.md scope re-cut, not yet registered — see 31.2-03/04/06/07/08/09-SUMMARY.md, pending a future close pass).
...
Derived from `38.0.1-CONTEXT.md` R-01..R-18, which in turn close the 19 remaining findings of the
Codex adversarial review of 2026-09-06. **These requirements change no doctrine** — D-01..D-56 stay
locked; each makes the code do what they already say. Three P0s (F1/F7/F10) shipped 2026-09-06 as a
quick task with migration 187 and are NOT requirements here.

**Split approved by the owner 2026-09-06.** 38.0.1 = slices A + B + E + the consent surface.
38.0.2 = transactional integrity + hygiene.

| ID | Requirement | Decision | Finding | Phase |
|---|---|---|---|---|
| WSR-01 | Member-side consent endpoint; the Member is the root of grant authority | R-01 | F6 | 38.0.1 |
| WSR-02 | `parent_grant_id` delegation lineage, re-validated at use time | R-01 | F6 | 38.0.1 |
| WSR-03 | Remove the workspace branch from the four child tables | R-02 | F2 | 38.0.1 |
| WSR-04 | SECURITY DEFINER read RPCs with explicit column allowlists | R-02 | F2 | 38.0.1 |
| WSR-05 | Narrow the `vault_projects` workspace branch; no custody mutation | R-02 | F3 | 38.0.1 |
| WSR-06 | Helper binds access to custody (`p.user_id = r.member_user_id`) | R-04 | F4 | 38.0.1 |
| WSR-07 | Owner-only promotion; no self-promotion; admins cannot touch owner rows | R-05 | F5 | 38.0.2 |
| WSR-08 | Two-sided ownership transfer (nominate + accept) | R-05 | F5 | 38.0.2 |
| WSR-09 | Transactional RPC: custody acceptance | R-06 | F9 | 38.0.2 |
| WSR-10 | Transactional RPC: invitation redemption | R-06 | F11 | 38.0.2 |
| WSR-11 | Transactional RPC: ownership / owner-floor | R-06 | F15 | 38.0.2 |
| WSR-12 | Transactional RPC: roster transitions | R-06 | F16 | 38.0.2 |
| WSR-13 | Audit insert inside the same transaction as its mutation | R-06 | F14 | 38.0.2 |
| WSR-14 | Evidence: workspace proposes, subject confirms, document mandatory | R-08 | F8 | 38.0.1 |
| WSR-15 | Enforce `effective_from <= now` at use time | R-08 | F8 | 38.0.1 |
| WSR-16 | Cohort gate on creation and `requireWorkspaceAccess` | R-07 | F17 | 38.0.2 |
| WSR-17 | One canonical live-membership definition incl. `expires_at` | R-11 | F12 | 38.0.1 |
| WSR-18 | Proposed relationships visible only to named Member + owner/admin | R-12 | F13 | 38.0.2 |
| WSR-19 | Redacted audit reads; no restricted PII in `changes` | R-13 | F14 | 38.0.2 |
| WSR-20 | Set-based paginated catalogue and roster RPCs | R-10 | F18 | 38.0.1 |
| WSR-21 | "Appears on" requires a contributor relationship naming the caller | R-09 | F20 | 38.0.2 |
...
**WSR-24 note:** pulled into 38.0.1 by the planner, and correctly. A NULL `relationship_id` makes
migration 186's helper match ANY accepted relationship in the workspace, which defeats WSR-06's
...
**Coverage (38.0.1):** 14 requirements across 14 plans / 6 waves — WSR-01..06, 14, 15, 17, 20, 22,

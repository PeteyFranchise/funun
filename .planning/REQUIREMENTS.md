...
| R6 | Phase 31.1 | 31.1-01 (migration live), 31.1-03 (aggregation done), 31.1-04 (tower render + hide-not-filter done); 31.1-06 pending | In progress — the All-tab leadership tower (coverage strip, Needs-an-AE queue, book table, By-AE grouping) renders at `/admin/client-partners`, server-gated (D-31.1-01 hide-not-filter machine-verified, `lib/admin/gate.test.ts`) + nav collapsed to one item (31.1-04-SUMMARY.md); the richer assign panel (D-31.1-05) not yet built |
...
| R7 | Phase 31.1 | 31.1-01 (migration live); 31.1-04, 31.1-06 pending | In progress — migration 128's `onboarding_tasks` table (D-07 handoff queue) applied to prod (31.1-01-SUMMARY.md); the assign panel/route and the auto-created-task insert on assignment not yet built |
| R8 | Phase 31.1 | 31.1-01 (migration live); 31.1-06 pending | In progress — migration 128's `onboarding_tasks` table applied to prod (31.1-01-SUMMARY.md); the locked intro-email + auto-created onboarding task insert on assignment not yet built |
...
- Phase 31 Slice-1 requirement IDs: **6** (R1, R2, R5, R10, R11, R12). Slice-2 IDs (R3, R4, R6–R9, R13, R14) → Phase 31.1, in progress (R3/R4/R6/R7/R8/R14 partial per rows above — migration 128 landed 2026-08-24 for all six; R3/R6 further advanced by 31.1-04's consolidated room 2026-08-24; R9/R13 not yet started).

...
- **A "What is Buffer?" explainer is required on the connect screen.** Funūn artists may not know Buffer — the connect step now leads with a plain-language block: what Buffer is (a social scheduling tool for IG/TikTok/X/Facebook/Threads), what connecting does (pushes the calendar into Buffer's queue so posts publish automatically, no CSV), and a link to buffer.com. Without it, the paste-key ask has no context.

...
stopped_at: Phase 38.0.1 SHIPPED AND VERIFIED. 16 plans merged, migrations 186-196 ALL APPLIED, all code deployed (origin/main == local), 499 suites / 5540 tests, tsc clean, npm run build passing. *** VERIFICATION COMPLETE: Part A (35 structural checks, 0 FAIL) and Part B (12 behavioural checks, ALL PASS) both ran against production 2026-09-07. Part B seeded six identities, enabled the D-56 kill switch, exercised the real authorization paths, tore down and restored the switch, all inside one atomic DO block; teardown confirmed switch OFF and tables empty. *** PART B FOUND AND FIXED A PRODUCTION BUG NOTHING STATIC COULD REACH: migration 139's guard_owner_immutable blocked transfer_vault_project_custody, so custody transfer had NEVER worked - migration 139 assumed triggers do not fire inside SECURITY DEFINER functions, which is false. Migration 196 adds a doubly-scoped exemption (TG_TABLE_NAME = 'vault_projects' AND current_user IN ('postgres')); public.works deliberately keeps the absolute guard. Applied and re-verified: B6 PASS, and B10 still refuses a raw UPDATE as authenticated, so the exemption did not over-widen. *** VERIFIED IN PRODUCTION: WSR-02, 03, 04, 05, 06, 17, 24, 25. NOT verified and deliberately so: WSR-01, 14, 15, 20, 22, 27, 28 are application-layer and SQL cannot prove them - they stay unit-tested pending plan 13's human check. *** D-56 KILL SWITCH IS OFF and must stay off until Phase 38.0.2. *** OPEN: the 57-box 38-RLS-SMOKE-CHECKLIST.md (Part B covered much of the adversarial matrix but not the UI surfaces); plan 13's human check; the kill-switch-before-auth ordering in the consent route (ORCHESTRATOR-NOTES item 1); and an OPEN PRODUCT QUESTION - workspace_members.role does not gate project access at all, so a guest reaches an attached project exactly as an owner does, because grants are per-relationship not per-member. Not a defect against any WSR as written. Phase 38.0.2 next (13 deferred requirements). Phase 37.1's 37-13 Task 3 hum test also open.
last_updated: "2026-09-07T00:00:00.000Z"
last_activity: 2026-09-07
last_activity_desc: "Phase 38.0.1 verified in production: Part A 35/35, Part B 12/12; migration 196 fixed a custody bug Part B found"
progress:
  total_phases: 42
  completed_phases: 33
  total_plans: 287

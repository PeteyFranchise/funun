...
status: board-clear
stopped_at: "Phase 28 Wave 1 done. Migrations 085 (industry cap + Green Room RLS) + 086 (buyer-branch fix) PUSHED live. Live smoke found 2 real bugs (pre-existing, surfaced by Phase 28): Bug 1 — handle_new_user can't see app_metadata.role at INSERT time (GoTrue applies it just after the users insert), so buyer/industry accounts get artist profiles; Bug 2 — 085's INLINE-EXISTS Green Room INSERT RLS rejects even valid artists. CORRECTIVE FIX built + committed, full suite 1449 green: Bug 1 = provisioning functions (createBuyerAccount deletes phantom profile/sub; provisionIndustryAccount sets member_type='industry' + guarded capability grant) reconcile post-createUser, trigger-timing-independent (commit cbdf405); Bug 2 = 087 SECURITY DEFINER helper (commit 8ccbc09, PUSHED). 087 push + re-smoke: a/b/d/e PASS (buyer no profile, industry posts opportunity 200, buyer RLS-rejected, @funun app-blocked) but (c) STILL FAILED — eligible artist/industry still 42501 on Green Room post. Full live diagnostic (real artist Bearer session) proved the INSERT is fine: auth.uid()=author, member_type=artist, is_green_room_eligible=true, NO restrictive/stacked policy — yet insert 42501s. REAL ROOT CAUSE (latent since 057/059, orthogonal to 085/087): app inserts via .insert().select() = INSERT..RETURNING; Postgres applies the SELECT policy to the returned row; that policy is green_room_can_view_post(id,auth.uid()), a STABLE fn that RE-QUERIES green_room_posts for the row, whose snapshot can't see the just-inserted tuple → false → RETURNING rejected 42501. FIX = migration 088 (commit 0f88a22, NOT pushed): rewrite green_room_posts_select_visible to affirm the author's own non-deleted row via a DIRECT author_id=auth.uid() predicate (no re-query) OR green_room_can_view_post; INSERT policy untouched. 5/5 text tests. AWAITING: owner via Codex — (1) CONFIRM with a return=minimal (no-RETURNING) Bearer insert as an artist → expect success (proves it's the RETURNING path); (2) if confirmed push 088; (3) re-smoke (c) artist+industry Green Room post via the REAL app route → expect success, and re-confirm (d) buyer rejected + (e) @funun blocked → 'approved'. THEN close Phase 28 (28-05-SUMMARY, gsd-verify, register INDUSTRY-01..07). NOTE: Phase 25 plans reserved 085/086 — renumber to 089+ before executing Phase 25 (085/086/087/088 now taken)."

...
Stopped at: Follow-up review (2026-08-09) — 27-CODEX-REVIEW.md's "Follow-up review" section: independent re-audit of the B3/H1/M5 fix commits (fd526b8/def0c0f) found the shared mintOrRotateInvite() claim was not actually atomic under concurrency, plus non-atomic write-result-ignored issues in the broadcast/convert routes calling it. Fixed via three commits (e3d2d54/b963ca2/16cb802): migration 101 (mint_or_rotate_artist_invite RPC, single row-locked INSERT...ON CONFLICT DO UPDATE) closes the rotation race + the reintroduced ILIKE wildcard; broadcast/route.ts now atomically claims each recipient before mint/send, resetting the claim on failure (retryable); convert/route.ts now requires an atomic converted_to_invite_at IS NULL claim before a first-time conversion sends, preserving H1's re-issue-on-expiry path for already-converted rows. 166 suites/1951 tests, tsc, next lint, next build all clean. Migration 101 is TEXT/TEST-ONLY, not pushed. NEXT = Wave 5 owner cutover (27-11): (1) confirm/seed owner bootstrap artist account, (2) provision Cloudflare Turnstile keys, (3) sign off 3 email templates, (4) [BLOCKING] supabase db push migrations 097+098+099+100+101 together, (5) smoke test. Migrations NOT yet pushed; gate NOT live. Then /gsd-docs-update to register INVITE-01..12, then /gsd-verify-work + /gsd-ship.

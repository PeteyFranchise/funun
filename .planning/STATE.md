...
status: pending
stopped_at: "Phase 27 waves 1-4 SHIPPED (10/11 plans on main, full suite green, /signup gate + /unsubscribe live-verified). Independent Codex review (27-CODEX-REVIEW.md) found 3 blockers + 2 high + 8 medium + 3 low pre-cutover; DB-gate findings B1/M1/M2/M3/M4/L1 fixed via migration 099 (fix-forward, 098 left unedited) + allowlist.ts/test updates + break-glass docs — 162 suites/1905 tests, tsc, lint, build all clean; migration 099 is TEXT/TEST-ONLY, not pushed. B2 (unsubscribe never set unsubscribed_at) fixed via a new token-authenticated POST /api/waitlist/unsubscribe — 163 suites/1912 tests, tsc, lint, build all clean; no migration involved. B3/H1/H2/M5/M6/L3 fixed 2026-08-09 (commits fd526b8/def0c0f/d5eed0d/6348363): new lib/invites/mintInvite.ts (mintOrRotateInvite) shared by admin issue-invite, convert, and broadcast routes — rotates an EXPIRED pending invite's token+expiry instead of reporting a stale duplicate (H1), and the broadcast route now mints/reuses/rotates each eligible recipient's own authorizing invite BEFORE sending (B3), tracking delivered/failed separately and stamping notified_reopen_at ONLY on successful delivery (M5). New migration 100 (upsert_artist_waitlist RPC, atomic INSERT...ON CONFLICT(LOWER(email)) DO UPDATE) replaces /api/waitlist's error-ignoring select-then-branch — every write error is now checked, {ok:true} never returned without a persisted row (H2); sanitizeWaitlistEntry() now requires non-empty name and caps email(254)/name(200) (L3). Collaborator invite email now escapes collaborator.name/joinUrl/inviteUrl via lib/email/esc.ts (M6). 166 suites/1946 tests, tsc, next lint, next build all clean; migrations 097/098/099/100 remain TEXT/TEST-ONLY, none pushed. L2 (enumeration/rate-limit) remains ACCEPTED for beta per review disposition. FOLLOW-UP REVIEW #1 fixed 2026-08-09 (commits e3d2d54/b963ca2/16cb802, 27-CODEX-REVIEW.md 'Follow-up review' section): an independent re-audit of the B3/H1/M5 fix commits themselves found they were not actually atomic under concurrency. New migration 101 (mint_or_rotate_artist_invite RPC, SECURITY DEFINER/search_path=''/service_role-only, single row-locked INSERT...ON CONFLICT(LOWER(email)) WHERE status='pending' DO UPDATE against migration 099's partial unique index) replaces mintInvite.ts's JS select-then-branch, closing a torn-read/write rotation race AND the reintroduced ILIKE wildcard (no more `.ilike()` call at all — exact LOWER(email) equality only) in one fix. broadcast/route.ts now atomically CLAIMS each recipient (conditional UPDATE...WHERE notified_reopen_at IS NULL, checked via select().maybeSingle()) BEFORE minting/sending, resetting the claim on mint/send failure so it stays retryable — closing a double-send race between overlapping broadcast runs. convert/route.ts now requires winning an atomic UPDATE...WHERE converted_to_invite_at IS NULL claim before a FIRST-TIME conversion sends, losing the race returns duplicate:true with no email; H1's re-issue-on-expiry path for already-converted rows is preserved, with its restamp write now checked instead of ignored. 166 suites/1951 tests, tsc, next lint, next build all clean; migration 101 is TEXT/TEST-ONLY, not pushed — applies after 100. FOLLOW-UP REVIEW #2 fixed 2026-08-09 (commits 0923f4a/45470b9/b0d0161, 27-CODEX-REVIEW.md 'Follow-up review #2' section): the two remaining MEDIUMs were about PARTIAL failure (process death mid-send, ambiguous provider timeout, lost RPC response), not pure concurrency. New migration 102 adds artist_waitlist.claim_token/claimed_at (broadcast's PER-ATTEMPT lease, distinct from notified_reopen_at, which is now stamped ONLY on a CAS-confirmed send) and invite_email_sent_at (convert's confirmed-sent marker, distinct from converted_to_invite_at bookkeeping). broadcast/route.ts: a fresh claim_token/claimed_at lease is set BEFORE mint/send (reclaimable via a lease-expiry OR-clause if the claiming process dies); mint/send failure releases the lease via a claim_token CAS; notified_reopen_at is stamped only via that same CAS on confirmed success; every send carries a stable day-scoped Resend idempotency key (artist-reopen-{waitlist_id}-{day}). convert/route.ts: duplicate-suppression is now keyed on invite_email_sent_at (not converted_to_invite_at/mint-state alone), so a retry after a lost mintOrRotateInvite() response correctly (re)sends using the reused invite instead of silently reporting a duplicate; a mint failure on a first-time claim releases it via a converted_to_invite_at CAS; every send carries a stable idempotency key (artist-convert-{waitlist_id}-{invite_token}). lib/email/index.ts's sendEmail() gained an optional idempotencyKey passthrough to the Resend SDK's send(payload, options) second argument. 167 suites/1959 tests, tsc, next lint, next build all clean; migration 102 is TEXT/TEST-ONLY, not pushed — applies after 101. FOLLOW-UP REVIEW #3 fixed 2026-08-09 (commits bff852f/feb63ec, 27-CODEX-REVIEW.md 'Follow-up review #3' section): two remaining MEDIUM correlation/stability gaps + one LOW observability gap, no HIGH/BLOCKER. NEW ISSUE 1 — convert's invite_email_sent_at was a boolean 'was ANY send ever confirmed' presence check with no correlation to WHICH token was delivered; since issue-invite/convert/broadcast all share mintOrRotateInvite() for the same email, the active token can rotate out from under a convert call, and the old check could silently suppress a resend of a token that was never actually delivered. New migration 103 adds artist_waitlist.invite_email_sent_token (stamped alongside invite_email_sent_at with the EXACT token confirmed sent); convert/route.ts's duplicate check is now a direct token comparison (row.invite_email_sent_token === mint.token) instead of a presence/state check, independent of mint.state. NEW ISSUE 2 — broadcast's Resend Idempotency-Key was day-scoped (artist-reopen-{id}-{day}), so a delayed retry or one straddling a UTC-midnight rollover computed a different key, defeating the key's purpose; now scoped to the row alone (artist-reopen-{id}), safe because each row can only ever be broadcast to once (notified_reopen_at IS NULL eligibility). LOW — both routes' claim-release UPDATE results were captured but never checked; now inspected for a genuine DB error vs. a benign CAS-miss and surfaced via logStaffAction (convert: dedicated artist_invite.convert_release_failed entry; broadcast: releaseIssues array attached to the run's artist_invite.broadcast entry) rather than console-logged or silently dropped. 167 suites/1965 tests, tsc --noEmit, next lint, next build all clean; migration 103 is TEXT/TEST-ONLY, not pushed — applies after 102. NEXT = Wave 5 owner cutover (27-11): (1) confirm/seed owner bootstrap artist account, (2) provision Cloudflare Turnstile keys, (3) sign off 3 email templates, (4) [BLOCKING] supabase db push migrations 097+098+099+100+101+102+103 together (flips gate LIVE — 098 alone re-introduces B1), (5) smoke test. Migrations NOT yet pushed; gate NOT live. Then /gsd-docs-update to register INVITE-01..12, then /gsd-verify-work + /gsd-ship."
last_updated: "2026-08-09T15:45:00.000Z"
last_activity: 2026-08-09
last_activity_desc: Phase 27 follow-up review #3 — fixed convert sent-token correlation (migration 103), broadcast idempotency-key stability, and claim/release-result observability in both routes (LOW)
progress:
  total_phases: 28
  completed_phases: 23
  total_plans: 173
...
Last session: 2026-08-09T14:30:00.000Z
Stopped at: Follow-up review #2 (2026-08-09) — 27-CODEX-REVIEW.md's "Follow-up review #2" section: the two remaining MEDIUMs (broadcast/convert partial-failure durability — process death mid-send, ambiguous provider timeout, lost RPC response) fixed via three commits (0923f4a/45470b9/b0d0161). lib/email/index.ts's sendEmail() gained an optional idempotencyKey passthrough to Resend. Migration 102 adds artist_waitlist.claim_token/claimed_at (broadcast's per-attempt lease, separate from the notified_reopen_at final-delivered marker) and invite_email_sent_at (convert's confirmed-sent marker, separate from converted_to_invite_at bookkeeping). broadcast/route.ts now leases each recipient with a reclaimable claim (lease-expiry OR-clause), releases it via claim_token CAS on mint/send failure, stamps notified_reopen_at only via that same CAS on confirmed success, and sends with a stable day-scoped idempotency key. convert/route.ts now keys duplicate-suppression on invite_email_sent_at (not converted_to_invite_at/mint-state), so a retry after a lost mint response resends using the reused invite instead of silently reporting a duplicate; a mint failure on a first-time claim releases it via CAS; sends carry a stable per-invite-token idempotency key. 167 suites/1959 tests, tsc, next lint, next build all clean. Migration 102 is TEXT/TEST-ONLY, not pushed. NEXT = Wave 5 owner cutover (27-11): (1) confirm/seed owner bootstrap artist account, (2) provision Cloudflare Turnstile keys, (3) sign off 3 email templates, (4) [BLOCKING] supabase db push migrations 097+098+099+100+101+102 together, (5) smoke test. Migrations NOT yet pushed; gate NOT live. Then /gsd-docs-update to register INVITE-01..12, then /gsd-verify-work + /gsd-ship.

...
status: board-clear
stopped_at: "Phase 28 Wave 1 done. Migrations 085 (industry cap + Green Room RLS) + 086 (buyer-branch fix) PUSHED live. Live smoke found 2 real bugs (pre-existing, surfaced by Phase 28): Bug 1 — handle_new_user can't see app_metadata.role at INSERT time (GoTrue applies it just after the users insert), so buyer/industry accounts get artist profiles; Bug 2 — 085's INLINE-EXISTS Green Room INSERT RLS rejects even valid artists. CORRECTIVE FIX built + committed, full suite 1449 green: Bug 1 = provisioning functions (createBuyerAccount deletes phantom profile/sub; provisionIndustryAccount sets member_type='industry' + guarded capability grant) reconcile post-createUser, trigger-timing-independent (commit cbdf405); Bug 2 = migration 087 replaces the inline EXISTS with a SECURITY DEFINER is_green_room_eligible() helper matching is_buyer_org_member (commit 8ccbc09, NOT pushed). AWAITING: owner pushes 087 via Codex + re-runs the 5 smoke checks (a buyer→no profile, b industry→can post opportunity, c artist+industry→Green Room post OK, d non-eligible→RLS reject, e @funun→app-block) → 'approved'. THEN close Phase 28 (28-05-SUMMARY results, gsd-verify, register INDUSTRY-01..07). NOTE: Phase 25 plans reserved 085/086 — renumber to 088+ before executing Phase 25 (085/086/087 now taken)."

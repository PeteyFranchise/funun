...
last_activity_desc: "Phase 38.0.3 plan 04 complete INCLUDING its review fix — placements-admin decoupled from the no_block RPC. Review caught a fail-open regression in the first cut: the replacement read ran on the caller's user-scoped client, where RLS policy blocks_select_own hides the owner-blocked-viewer direction, so blocked users were shown placements. Now reads blocks with a memoized SERVICE-ROLE client (the privilege equivalent of the SECURITY DEFINER function it replaced), pinned by behavioural + structural tests across 4 suites. tsc clean, 6682/6682 tests green. NOT PUSHED. MUST BE MERGED TO main AND DEPLOYED before the migration that relocates no_block is applied."
progress:
  total_phases: 42
  completed_phases: 33
  total_plans: 287

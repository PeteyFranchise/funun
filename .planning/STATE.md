...
stopped_at: Phase 38.0.1 SHIPPED — all 16 plans merged, migrations 186-195 ALL APPLIED, and 78 commits deployed to main (97328b3f..d127819e) on 2026-09-07. 498 suites / 5510 tests green, tsc clean, npm run build passing. Migrations and code are now in sync; origin/main == local HEAD. *** VERIFIED LIVE by anon RPC probe (SQLSTATE 42501 = exists and locked down, vs PGRST202 = missing): transfer_vault_project_custody, workspace_read_tracks, workspace_read_documents, workspace_grant_lineage_live; and workspace_permission_requests returns [] not 404, so the table exists with RLS filtering anon correctly. The custody-transfer production 500 is RESOLVED. *** WHAT REMAINS IS VERIFICATION, NOT BUILDING. Plan 11's Task 3 steps 6-11 have NOT been run: (6) WSR-03/04 column allowlist as a workspace caller, (7) WSR-06 custody binding drops access on the next read, (8) WSR-17 expired seat refused identically by RLS and the API gate, (9) EXPLAIN ANALYZE for no 42P17 and index scans, (10) the 57-box 38-RLS-SMOKE-CHECKLIST.md outstanding since Phase 38, (11) Phase 21 Shared-with-me regression. All need authenticated sessions and the D-56 kill switch briefly ON. Until they run, no WSR row may be marked Verified. *** D-56 KILL SWITCH MUST STAY OFF until Phase 38.0.2 lands (R-03, R-07) - the feature is deployed DARK. *** STILL OPEN: project-scoped asks (ORCHESTRATOR-NOTES item 4, recommendation is refuse); kill-switch-before-auth ordering in the consent route (item 1); plan 13's human check. *** A BUILD BREAK WAS CAUGHT PRE-DEPLOY: two eslint-disable directives naming @typescript-eslint/no-var-requires, a plugin this project never loads, failed next build at the lint step while tsc and 5510 tests stayed green. Run npm run build before any deploy; tsc --noEmit does not cover the lint step. Phase 37.1's 37-13 Task 3 hum test also open.
last_updated: "2026-09-07T00:00:00.000Z"
last_activity: 2026-09-07
last_activity_desc: "Phase 38.0.1 shipped: migrations 186-195 applied, 78 commits deployed; live verification steps 6-11 remain"
progress:
  total_phases: 42
  completed_phases: 33
  total_plans: 287

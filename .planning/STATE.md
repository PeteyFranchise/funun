...
stopped_at: Phase 38.0.1 SHIPPED AND VERIFIED (migrations 186-196 applied; Part A 35/35 structural, Part B 12/12 behavioural). *** PHASE 38.0.2 EXECUTING — 17 plans / 6 waves. WAVES 1-3 MERGED 2026-09-07 (plans 01-12) plus the R-23 raw-table quick fix: 518 suites / 6039 tests green, tsc clean. MIGRATION 197 IS COMPLETE (162 tests); 198 still open, plan 11 closes it. R-23 escalated at review: the blocked-to-refused collapse lived only in the read function while the raw policy still admitted owners and admins, so workspace access to the raw roster table was removed entirely and all workspace reads now route through workspace_roster_page - same move 38.0.1 made for the four child tables. R-12 verified intact: the function serves proposed rows to owners and admins, collapsed. Migration 197 has 113 tests (25 mutation cycles, all caught), 198 has 77 with 8 FOR NO KEY UPDATE clauses and zero FOR UPDATE. Shipped: ownership-transfer state machine, D-55 cohort gate (two env vars, DEFAULT CLOSED), owner-management + guest role-floor predicates, appears-on credit fix, and migrations 197 and 198 OPENED (both deliberately incomplete; 197 closes at plan 09, 198 at plan 11). *** HIGHEST-CONSEQUENCE CARRY-FORWARD: plan 13 must make workspace_create a postgres-owned SECURITY DEFINER function - 197 section (c) refuses every non-definer owner INSERT, so a non-definer create path means NOBODY CAN CREATE A WORKSPACE once 197 applies, silently. See 38.0.2-ORCHESTRATOR-NOTES.md. *** Plan 09 must add the SQL half of the R-20 role floor. *** OWNER ACTION: .env.example needs two cohort vars added by hand (agents are permission-denied on that path); block is in 38.0.2-02-SUMMARY.md; leaving them unset is SAFE because the gate defaults closed. *** Migrations 197-198 are AUTHORED, NOT APPLIED, and are human-gated. D-56 kill switch OFF and must stay off until this phase ships. Waves 2-6 remain; owner gates are the 197 review (wave 3), the 198 review (wave 4) and the joint push (wave 6). *** Codex is building the Playbook rich-content model in this same working tree and holds its commits until Phase 38 finishes; migrations 201-202 are reserved for it. Do not edit anything under playbook/. *** Also open from 38.0.1: the 57-box RLS smoke checklist, plan 13's human check, and the kill-switch-before-auth ordering in the consent route.
last_updated: "2026-09-07T00:00:00.000Z"
last_activity: 2026-09-07
last_activity_desc: "Phase 38.0.2 wave 3 merged; migration 197 complete; R-23 raw-table leak closed"
progress:
  total_phases: 42
  completed_phases: 33
  total_plans: 287

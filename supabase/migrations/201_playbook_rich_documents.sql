...
-- is never represented by a grant row.
INSERT INTO public.playbook_rooms (key, label, sort_order, sensitive, coming_soon) VALUES
  ('talent-services',      'Talent Services & Member Success',       8,  false, false),
  ('sync-licensing',       'Sync & Licensing',                       9,  false, false),
  ('rights-legal',         'Rights, Legal & Contract Operations',   10,  true,  false),
  ('catalogue-operations', 'Catalogue Operations',                  11,  false, false),
  ('finance',              'Finance',                               12,  true,  false),
  ('marketing',            'Marketing',                             13,  false, false),
  ('trust-safety',         'Trust & Safety',                        14,  true,  false),
  ('support-operations',   'Support Operations',                    15,  false, false)
ON CONFLICT (key) DO UPDATE SET
  label = EXCLUDED.label,
  sort_order = EXCLUDED.sort_order,
  sensitive = EXCLUDED.sensitive;

WITH internal_rooms(key) AS (
  VALUES ('talent-services'), ('sync-licensing'), ('catalogue-operations'), ('marketing'), ('support-operations')
), roles(role) AS (
  VALUES ('ae'), ('bd'), ('anr'), ('it'), ('legal'), ('tms'), ('accounting'), ('marketing')
), grants(room_key, role) AS (
  SELECT internal_rooms.key, roles.role FROM internal_rooms CROSS JOIN roles
  UNION ALL SELECT 'rights-legal', 'legal'
  UNION ALL SELECT 'finance', 'accounting'
)
INSERT INTO public.playbook_room_role_grants (room_id, role)
SELECT room.id, grants.role
FROM grants
JOIN public.playbook_rooms room ON room.key = grants.room_key
ON CONFLICT (room_id, role) DO NOTHING;

UPDATE public.playbook_rooms
SET coming_soon = false
WHERE key IN ('company-wide', 'ar', 'business-development', 'ae-sales', 'it-team', 'tms', 'leadership');

-- Trust & Safety starts Leadership-only because no dedicated staff role exists
-- yet. Access must be explicitly delegated later; it is never inferred.
WITH subgroup(room_key, key, label, sort_order) AS (
  VALUES
    ('company-wide', 'organizational-doctrine', 'Organizational Doctrine', 30),
    ('company-wide', 'cross-functional-operations', 'Cross-Functional Operations', 40),
    ('ar', 'role-doctrine', 'Role Doctrine', 30),
    ('ae-sales', 'role-doctrine', 'Role Doctrine', 30),
    ('business-development', 'role-doctrine', 'Role Doctrine', 30),
    ('talent-services', 'role-doctrine', 'Role Doctrine', 30),
    ('sync-licensing', 'role-doctrine', 'Role Doctrine', 30),
    ('rights-legal', 'role-doctrine', 'Role Doctrine', 30),
    ('catalogue-operations', 'role-doctrine', 'Role Doctrine', 30),
    ('finance', 'role-doctrine', 'Role Doctrine', 30),
    ('marketing', 'role-doctrine', 'Role Doctrine', 30),
    ('it-team', 'role-doctrine', 'Role Doctrine', 30),
    ('leadership', 'leadership-doctrine', 'Leadership Doctrine', 30),
    ('leadership', 'workforce-planning', 'Workforce Planning', 40),
    ('leadership', 'launch-planning', 'Launch Planning', 50),
    ('tms', 'role-doctrine', 'Role Doctrine', 30),
    ('tms', 'training-enablement', 'Training & Enablement', 40),
    ('trust-safety', 'role-doctrine', 'Role Doctrine', 30),
    ('support-operations', 'role-doctrine', 'Role Doctrine', 30)
)
INSERT INTO public.playbook_sub_groups (room_id, key, label, sort_order)
SELECT room.id, subgroup.key, subgroup.label, subgroup.sort_order
FROM subgroup
JOIN public.playbook_rooms room ON room.key = subgroup.room_key

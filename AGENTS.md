...
- `@anthropic-ai/sdk` 0.52.0 - Codex API integration for AI-powered tools (PitchPlug, contract verification)
...
Before using Edit, Write, or other file-changing tools, start work through a GSD command when the tooling is available so planning artifacts and execution context stay in sync.

Use these entry points when available:

- `/gsd-quick` for small fixes, doc updates, and ad-hoc tasks
- `/gsd-debug` for investigation and bug fixing
- `/gsd-execute-phase` for planned phase work

If the GSD command/tooling is not installed in this checkout, use a manual planning fallback before editing:

- Create `.planning/quick/<YYMMDD-task-slug>/PLAN.md` with scope, assumptions, and verification steps.
- After implementation, create or update `.planning/quick/<YYMMDD-task-slug>/SUMMARY.md` with completed changes and verification results.
- Mention in the final response that the manual fallback was used because GSD tooling was unavailable.

Do not make direct repo edits outside a GSD command or this manual planning fallback unless the user explicitly asks to bypass planning.
<!-- GSD:workflow-end -->

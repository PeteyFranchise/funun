...
// experience against a fabricated profile with no sign-in. The real app does
// NOT render this — it composes the same pieces across three tab routes under
// app/(artist)/settings/, with the provider mounted in the layout.
//
// Its `{ profile }` signature is load-bearing for that preview page. Do not
// change it without editing app/profile-preview/page.tsx, and note that
// __tests__/profile-preview-route.test.ts text-locks that page's guards.
...
function PublicProfileSectionsInline() {
...
function PrivacySettingsFormInline() {
  const {
    visibilityForm,
    setVisibilityForm,
  ...
  async function handleVisibilitySave(e: React.FormEvent) {
    ...
    {/* ── Privacy Settings (SAFETY-04) ────────────────────────────────
        Separate form/endpoint from the main profile save — these two
        columns have no authenticated UPDATE grant at all (migration 058),
        so they must go through the dedicated service-role-backed
        /api/profile/visibility route.
    ────────────────────────────────────────────────────────────── */}
  return (
  ...
  return (
    <SettingsFormProvider profile={profile}>
      <div className="space-y-12">
        ...
        <PublicProfileSectionsInline />
        <PrivacySettingsFormInline />
      </div>
    </SettingsFormProvider>
  )
}

...
export async function POST(request: Request) {
  ...
  if (room.error) return NextResponse.json({ error: room.error.message }, { status: 500 })
  ...
  if (entry.error) return NextResponse.json({ error: entry.error.message }, { status: 500 })

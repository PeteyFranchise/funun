...
export async function POST(request: Request) {
  ...
  if (entries.error) return NextResponse.json({ error: entries.error.message }, { status: 500 }); if ((entries.data ?? []).length !== new Set(parsed.data.entryIds).size) return NextResponse.json({ error: 'Every learning step must be published guidance in this room' }, { status: 400 })
  ...
  if (path.error) { if (isPlaybookEnablementSchemaMissing(path.error)) return NextResponse.json({ error: 'Learning paths are built but not activated yet.' }, { status: 503 }); return NextResponse.json({ error: path.error.message }, { status: 500 }) }
  ...
  const stepWrite = await service.from('playbook_learning_path_steps').insert(steps); if (stepWrite.error) { await service.from('playbook_learning_paths').delete().eq('id', path.data.id); return NextResponse.json({ error: stepWrite.error.message }, { status: 500 }) }
  const assignment = await service.from('playbook_learning_assignments').insert({ path_id: path.data.id, target_kind: 'role', target_role: parsed.data.targetRole, target_user_id: null, due_at: parsed.data.dueAt, assigned_by: auth.user.id }); if (assignment.error) { await service.from('playbook_learning_paths').delete().eq('id', path.data.id); return NextResponse.json({ error: assignment.error.message }, { status: 500 }) }

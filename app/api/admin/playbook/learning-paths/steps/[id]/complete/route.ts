...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const auth = await requireStaff(ALL_STAFF_ROLES); if ('error' in auth) return NextResponse.json({ error: auth.error }, { status: auth.status })
  ...
  const service = createServiceClient(); const roles = getStaffRoles(auth.user)
  ...
  if (assignments.error) return NextResponse.json({ error: 'Request could not be completed.' }, { status: 500 }); if (!(assignments.data ?? []).some(target => staffTargetApplies(target, auth.user.id, roles))) return NextResponse.json({ error: 'This learning path is not assigned to you' }, { status: 403 })
  ...
  if (step.data.knowledge_prompt) { const attempt = await service.from('playbook_knowledge_attempts').insert({ step_id: id, user_id: auth.user.id, response: parsed.data.response, self_confirmed: parsed.data.selfConfirmed }); if (attempt.error) return NextResponse.json({ error: 'Request could not be completed.' }, { status: 500 }) }
  const write = await service.from('playbook_learning_step_completions').upsert({ step_id: id, user_id: auth.user.id }, { onConflict: 'step_id,user_id' }); if (write.error) return NextResponse.json({ error: 'Request could not be completed.' }, { status: 500 })

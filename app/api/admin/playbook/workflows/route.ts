...
export async function POST(request: Request) {
  ...
  if (parsed.data.action === 'create_template') {
    ...
    if (template.error) { if (isPlaybookEnablementSchemaMissing(template.error)) return NextResponse.json({ error: 'Workflow automation is built but not activated yet.' }, { status: 503 }); return NextResponse.json({ error: template.error.message }, { status: 500 }) }
    ...
    if (steps.error) { await service.from('playbook_workflow_templates').delete().eq('id', template.data.id); return NextResponse.json({ error: steps.error.message }, { status: 500 }) }
  ...
  const run = await service.from('playbook_workflow_runs').insert({ template_id: template.data.id, context_type: parsed.data.contextType, context_id: parsed.data.contextId, context_label: parsed.data.contextLabel, started_by: auth.user.id, owner_id: auth.user.id }).select('id').single(); if (run.error) return NextResponse.json({ error: run.error.message }, { status: 500 })
  const steps = await service.from('playbook_workflow_template_steps').select('id, label, required').eq('template_id', template.data.id).order('sort_order'); const tasks = await service.from('playbook_workflow_run_tasks').insert((steps.data ?? []).map(step => ({ run_id: run.data.id, template_step_id: step.id, label: step.label, required: step.required }))); if (tasks.error) { await service.from('playbook_workflow_runs').delete().eq('id', run.data.id); return NextResponse.json({ error: tasks.error.message }, { status: 500 }) }

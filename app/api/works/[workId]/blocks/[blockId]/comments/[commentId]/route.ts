...
type RouteContext = {
  params: Promise<{ workId: string; blockId: string; commentId: string }>
}

const ResolutionSchema = z.object({ resolved: z.boolean() }).strict()

export async function PATCH(request: Request, { params }: RouteContext) {
  const { workId, blockId, commentId } = await params
  ...
  if (await checkRateLimit(`work-comment-resolution:${user.id}`, { maxAttempts: 120, windowMs: 15 * 60 * 1000 })) {

// ─── Incident Runbook (IT-room doc page) ────────────────────────────────
// Renders docs/observability/RUNBOOK.md via MarkdownDoc — the .md file
// stays the single source of truth (D-10). Self-guarded to the
...
const CRUMB = IT_SUBPAGES.find((p) => p.slug === 'runbook')!.label
...
export default async function RunbookPage() {
  ...
  const md = await readObservabilityDoc(DOC_PAGE_FILE['runbook'])

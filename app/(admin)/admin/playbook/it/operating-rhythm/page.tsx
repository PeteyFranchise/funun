// ─── Operating Rhythm (IT-room doc page) ────────────────────────────────
// Renders docs/observability/OPERATING-RHYTHM.md via MarkdownDoc — the .md
// file stays the single source of truth (D-10). Self-guarded to the
// data-driven it-team room grant before any content read (fail closed,
// D-02, T-33-01). Re-pointed from the Phase 33 hardcoded
...
const CRUMB = IT_SUBPAGES.find((p) => p.slug === 'operating-rhythm')!.label
...
export default async function OperatingRhythmPage() {
  ...
  const md = await readObservabilityDoc(DOC_PAGE_FILE['operating-rhythm'])
